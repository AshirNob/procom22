...
function get_card(bid, cat,cont_id) {
    ...
    $.ajax({
        url: 'api/get-card?bid='+bid+'&cat='+cat,
        type: "GET",
        success: function (res) {
           
            var cards = JSON.parse(res);
            if(cat==1){localStorage.setItem("todo",JSON.stringify(cards))}
            else if(cat==2){ localStorage.setItem("progress",JSON.stringify(cards))}
            else if(cat==3){ localStorage.setItem("review",JSON.stringify(cards))}
            else{ localStorage.setItem("done",JSON.stringify(cards))}
...
$.ajax({
    url:'api/update-card?cid='+cid+"&status="+status,
    type:"GET",
    success:function (res){
        console.log(res);
    }
