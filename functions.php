...
if(isset($_POST["sbmt_sign_up"])){
    ...
    if($res){    
        echo "<script>window.location.href='signup?msg=1'</script>";
    }else{
        echo "<script>window.location.href='error?msg=Email already registered&url=signup'</script>";
    }
}
...
if(mysqli_num_rows($res)>0){
    ...
    $_SESSION["role"]="User";
    echo "<script>window.location.href='my-boards'</script>";
}else{
    echo "<script>window.location.href='error?msg=Can't find Your Account&url=index'</script>";
}
}
...
if($res){
    ...
    echo "<script>window.location.href='error?msg=Can't Create Board Right now (-_-)&url=my-boards'</script>";
}
}else{
    echo "<script>window.location.href='error?msg=Session lost please login again&url=index'</script>";
}
}
...
if($res_){
    echo "<script>window.location.href='board?bid=$fk_board'</script>";
}else{
    echo "<script>window.location.href='error?msg=Opps Session lost &url=my-boards'</script>";
}
...
if(mysqli_num_rows($res_c)>0){
    echo "<script>window.location.href='error?msg=User already in this board&url=my-boards'</script>";
}else{
